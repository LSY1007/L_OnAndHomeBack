...
public class ReviewRestController {
    ...
    public ResponseEntity<Map<String, Object>> getMyReviews(
        ...
        try {
            ...
            log.info("User 정보 - id: {}, username: {}, userId: {}", userIdLong, username, userId);
            
            // 여러 방법으로 리뷰 조회 (Product fetch join 사용)
            java.util.Set<Long> existingIds = new java.util.HashSet<>();
            List<Review> allReviews = new ArrayList<>();
            
            // 1. user_id로 조회 (Product fetch join)
            List<Review> reviewList1 = reviewRepository.findByUserIdWithProduct(userIdLong);
            log.info("user_id({})로 조회된 리뷰 개수: {}", userIdLong, reviewList1.size());
            for (Review review : reviewList1) {
                if (!existingIds.contains(review.getId())) {
                    allReviews.add(review);
                    existingIds.add(review.getId());
                    log.info("리뷰 추가 (user_id) - id: {}, username: {}, author: {}", 
                            review.getId(), review.getUsername(), review.getAuthor());
                }
            }
            
            // 2. username으로 조회 (Product fetch join)
            List<Review> reviewList2 = reviewRepository.findByUsernameWithProduct(username);
            log.info("username({})으로 조회된 리뷰 개수: {}", username, reviewList2.size());
            for (Review review : reviewList2) {
                if (!existingIds.contains(review.getId())) {
                    allReviews.add(review);
                    existingIds.add(review.getId());
                    log.info("리뷰 추가 (username) - id: {}, username: {}, author: {}", 
                            review.getId(), review.getUsername(), review.getAuthor());
                }
            }
            
            // 3. author로 조회 (Product fetch join)
            List<Review> reviewList3 = reviewRepository.findByAuthorWithProduct(username);
            log.info("author({})로 조회된 리뷰 개수: {}", username, reviewList3.size());
            for (Review review : reviewList3) {
                if (!existingIds.contains(review.getId())) {
                    allReviews.add(review);
                    existingIds.add(review.getId());
                    log.info("리뷰 추가 (author) - id: {}, username: {}, author: {}", 
                            review.getId(), review.getUsername(), review.getAuthor());
                }
            }
            
            // 4. userId(String)를 username으로 조회 (Product fetch join)
            List<Review> reviewList4 = reviewRepository.findByUsernameWithProduct(userId);
            log.info("username({})으로 조회된 리뷰 개수: {}", userId, reviewList4.size());
            for (Review review : reviewList4) {
                if (!existingIds.contains(review.getId())) {
                    allReviews.add(review);
                    existingIds.add(review.getId());
                    log.info("리뷰 추가 (userId as username) - id: {}, username: {}, author: {}", 
                            review.getId(), review.getUsername(), review.getAuthor());
                }
            }
            ...
            for (Review review : allReviews) {
                ...
                if (review.getReplies() != null && !review.getReplies().isEmpty()) {
                    List<ReviewReply> replyList = new ArrayList<>(review.getReplies());
                    dto.setReply(replyList.get(0).getContent());
                }
    ...
    public ResponseEntity<Map<String, Object>> updateReviewWithImages(
            ...
            @RequestParam(required = false) List<MultipartFile> images,          // 새로 추가할 이미지들
            @RequestParam(required = false, name = "deleteImageIds") List<Long> deleteImageIds, // 삭제할 이미지 id 목록
            @RequestHeader(value = "Authorization", required = false) String authHeader,
            HttpSession session
    ) {
        Map<String, Object> response = new HashMap<>();
        try {
            ...
            // 6) 최신 리뷰 다시 조회 → 최신 이미지/내용 DTO 변환
